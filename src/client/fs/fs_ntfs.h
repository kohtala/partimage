...
class CNtfsPart : public CFSBase
{
 public:
  ...
  int readFilenameAttribute(BYTE *cAttribData, const char *szFilename);
  ...
  int searchForRecordFilename(BYTE *cFileRecord, const char *szFilename, char *szUnicodeName, int nUnicodeNameLen);
  int checkFilenameForRecordIs(BYTE *cRecordData, const char *szFilename, bool bShowDebug = true);
  int readVolumeLabel();
