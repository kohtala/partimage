...
CInterfaceNone::CInterfaceNone(char *a_optstring) : CInterface(true) {
  ...
  while (*where) {
    ...
    if (!equals) {
      fprintf(stderr,i18n("partimage: no equals in `%s'\n"),where);
...
void CInterfaceNone::invalid_programmed_response() {
  fprintf(stderr,i18n("partimage: invalid programmed response.\n"));
  flusherr();
  exit(8);
}

char *CInterfaceNone::lookup(const char *title, const char *text, char *deflt) {
  ...
  fprintf(stderr,i18n("partimage: %s (%s) no default\n"), title, text);
  exit(8);
}
...
void CInterfaceNone::message_only(const char *kind, const char *title,
				  char const *format, va_list al,
				  char *result) {
  fprintf(stderr,i18n("partimage: %s [%s]\npartimage:  "), title, kind);
  vfprintf(stderr,format,al);
  fputc('\n',stderr);
  if (result) fprintf(stderr,i18n("partimage:  => %s\n"),result);    
  flusherr();
}

#define MB_1(KInd,Kind)							\
  void CInterfaceNone::msgBox##KInd(const char *title, const char *text, ...) {	\
    va_list al;								\
    va_start(al,text);							\
...
void CInterfaceNone::msgBoxError(const char *title, ...) {
  va_list al;						
  va_start(al,title);					
...
#define MB_2(One,Other,ONE,OTHER)					  \
  int CInterfaceNone::msgBox##One##Other(const char *title, const char *text, ...) {  \
    char *result= lookup(title,text,(char *) "(unspecified)");			  \
    va_list al;								  \
    va_start(al,text);							  \
...
void CInterfaceNone::showErrorInternal(const char *text) {
  fprintf(stderr,i18n("partimage: internal error: %s\n"), text);
  flusherr();
}

void CInterfaceNone::askDescription(char * szDescription,
					 DWORD size) {
  askText((char *)"Description",(char *)"Description",szDescription,size);
}
...
int CInterfaceNone::askText(char *message, char *title,
  ...
  if (strlen(result) >= size) {
    fprintf(stderr,i18n("partimage: %s (%s) too long (>%lu)\n"), title, message,
	    (unsigned long)size);
    exit(12);
  }
...
void CInterfaceNone::StatusLine(char * str) {
  fprintf(stderr,i18n("partimage: status: %s\n"), str);
  flusherr();
}

#define DONT(rt,what,args)				\
  rt CInterfaceNone::what args {			\
    fprintf(stderr, i18n("partimage: "));	\
	fprintf(stderr, #what);			\
	fprintf(stderr, i18n(" not supported in CInterfaceNone\n"));	\
    exit(8);						\
  }
